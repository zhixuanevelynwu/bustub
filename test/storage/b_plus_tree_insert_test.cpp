...
TEST(BPlusTreeTests, GetValueRecurseTest) {
  ...
  const auto node = tree.GetBPlusTreePage(pid);
  index_key.SetFromInteger(4);
  tree.GetValueRecurse(node, index_key, nullptr);  // This should not result in an infinite loop
}
...
TEST(BPlusTreeTests, SmallBufferPoolTest) {
  ...
  auto disk_manager = std::make_unique<DiskManagerUnlimitedMemory>();
  auto *bpm = new BufferPoolManager(4, disk_manager.get());
  // create and fetch header_page
  page_id_t page_id;
  auto header_page = bpm->NewPage(&page_id);
  // create b+ tree
